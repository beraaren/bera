...
**Takım Adı:** [Takım Adı Eklenecek]  
**Takım Üyeleri:** [Üye Bilgileri Eklenecek]  
...
- **Video İşleme Süresi:** 8 karenin çıkartılması ve 4x2 gridin oluşturulması ortalama **<1 sn** sürmektedir.
- **Model Inference Süresi (Vulkan):** Çift GPU (AMD + NVIDIA) optimizasyonu ile modelin inference süresi yerel ortamda optimize edilmiştir. 
- **Bellek ve Kaynak Kullanımı:** GGUF (Q8_0) kullanımı sayesinde 7B model yaklaşık 7.5 - 8 GB arası bir VRAM tüketmekte olup, çoklu GPU ile yük dağıtımı başarıyla sağlanmıştır.
- **JSON Format Uyumluluğu:** Model prompt mühendisliği (prompt engineering) ile %90'ın üzerinde hatasız, parse edilebilir JSON çıktısı üretmektedir.
